...
var readImageInstance = file => new Promise(async resolve =>
{
...
{
  return _.reduce(_.pickBy(datas), (result, data) =>
  {
    return result.concat(data[key]
      ? getAllSubNodeID(data[key])
      : [data.id])
  }, [])
}

export default {
  readImage, getAllSubNodeID, readImageInstance
}